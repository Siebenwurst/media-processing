...
@frozen public struct CRC32: Hashable, Sendable {
    @_documentation(visibility: internal)
    public static let table: [UInt32] = (0 ..< 256).map { i in
    ...
    @inlinable public init(checksum: UInt32 = 0) {
        self.checksum = checksum
    }

    /// Computes the checksum of the provided message.
    @inlinable public init(hashing message: borrowing some Sequence<UInt8>) {
        self.init()
        self.update(with: message)
    }

    /// Returns a new checksum by hashing the provided message into the current checksum.
    @inlinable public func updated(with message: borrowing some Sequence<UInt8>) -> Self {
        var checksum = self
    ...
    @inlinable public mutating func update(with message: borrowing some Sequence<UInt8>) {
        self.checksum = ~message.reduce(~self.checksum) { state, byte in
            Self.table[Int(UInt8(truncatingIfNeeded: state) ^ byte)] ^ state >> 8
        }
    }
}
