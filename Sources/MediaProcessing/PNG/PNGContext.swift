...
struct PNGContext {
    ...
    private(set) var image: PNGImage

    private var decoder: PNGDecoder
}
...
extension PNGContext {
    ...
    mutating func push(ancillary chunk: (type: PNGChunkIdentifier, data: [UInt8])) throws {
        switch chunk.type {
        case .tIME:
            try PNGMetadata.unique(assign: chunk.type, to: &self.image.metadata.time) {
        ...
        case .CgBI, .IHDR, .PLTE, .bKGD, .tRNS, .hIST,
            ...
            throw PNGImage.DecodingError.unexpected(chunk: chunk.type, after: .IDAT)
        case .IEND:
            guard self.decoder.continue == nil else {
                throw PNGImage.DecodingError.incompleteImageDataCompressedDatastream
            }
        default:
