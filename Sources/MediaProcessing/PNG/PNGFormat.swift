...
extension PNGFormat {
    ...
    func validate() -> Self {
        let count = 1 << min(self.pixel.depth, 8)
        ...
        switch self {
        case .bgr8(let palette, _, _),
            .bgra8(let palette, _):
            max.index = palette.count - 1
        case .rgb8(let palette, _, _),
            .rgb16(let palette, _, _),
            .rgba8(let palette, _),
            .rgba16(let palette, _):
            max.index = palette.count - 1
        case .indexed1(let palette, _),
            ...
            guard !palette.isEmpty else {
                fatalError("\(PNGImage.ParsingError.invalidPaletteCount(0, max: count))")
            }
            max.index = palette.count - 1
        default:
            max.index = -1
        ...
        guard max.index < max.count else {
            fatalError("\(PNGImage.ParsingError.invalidPaletteCount(max.index + 1, max: max.count))")
        }
        ...
        case .v1(let fill?, _),
            .v2(let fill?, _),
            .v4(let fill?, _):
            let fill = UInt16(fill)
            guard fill <= max.sample else {
                fatalError("\(PNGImage.ParsingError.invalidBackgroundSample(fill, max: max.sample))")
            }
        case .indexed1(_, let i?),
            .indexed2(_, let i?),
            .indexed4(_, let i?),
            .indexed8(_, let i?):
            guard i <= max.index else {
                fatalError("\(PNGImage.ParsingError.invalidBackgroundIndex(i, max: max.index))")
            }
        default:
            break
        }
        ...
        switch self {
        case .v1(_?, key: let key?),
            .v2(_?, key: let key?),
            .v4(_?, key: let key?):
            let key = UInt16(key)
            guard key <= max.sample else {
                fatalError("\(PNGImage.ParsingError.invalidTransparencySample(key, max: max.sample))")
            }
        default:
            break
        }
    ...
    static func recognize(
        ...
        case .v1, .v2, .v4, .v8, .v16:
            guard palette == nil else {
                fatalError("\(PNGImage.ParsingError.unexpectedPalette(pixel: pixel))")
            }
            let f: UInt16?, k: UInt16?
        ...
        case .indexed1, .indexed2, .indexed4, .indexed8:
            ...
            case .palette(let alpha):
                guard alpha.count <= solid.entries.count else {
                    fatalError("\(PNGImage.ParsingError.invalidTransparencyCount(alpha.count, max: solid.entries.count))")
                }
        ...
        case .va8, .va16:
            ...
            guard transparency == nil else {
                fatalError("\(PNGImage.ParsingError.unexpectedTransparency(pixel: pixel))")
            }
        ...
        case .rgba8, .rgba16:
            ...
            switch (standard, pixel) {
            case (.common, .rgba8):
                format = .rgba8(palette: palette, fill: f.map{ (.init($0.r), .init($0.g), .init($0.b)) })
            case (.ios, .rgba8):
                format = .bgra8(palette: palette.map{ ($0.b, $0.g, $0.r) }, fill: f.map{ (.init($0.b), .init($0.g), .init($0.r)) })
            case (_, .rgba16):
