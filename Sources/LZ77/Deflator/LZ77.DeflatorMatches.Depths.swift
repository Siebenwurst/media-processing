...
extension LZ77.DeflatorMatches.Depths {
    ...
    private static let `default`: [UInt8] =
        // literal cost: 8.25 bps
        .init(repeating: 33, count: 256)
        // base run composite cost: 7.5 bps
        + (3 ... 258).map { (run: Int) -> UInt8 in
            30 + .init(LZ77.Composites[run: LZ77.Decades[run: run]].extra) << 2
        }
        // base distance composite cost: 4.75 bps
        + (0 ... 29).map { (decade: UInt8) -> UInt8 in
    ...
    mutating func update(runliteral: LZ77.HuffmanTree<UInt16>, distance: LZ77.HuffmanTree<UInt8>) {
        for (length, level): (UInt8, Range<Int>) in zip(1 ... 15, runliteral.levels) {
            for symbol: UInt16 in runliteral.symbols[level] {
                if symbol < 256 {
                    ...
                    let decade: (extra: UInt16, base: UInt16) = LZ77.Composites[run: .init(truncatingIfNeeded: symbol)]
