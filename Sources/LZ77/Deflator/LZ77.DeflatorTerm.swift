...
extension LZ77.DeflatorTerm {
    ...
    static let end: Self = .init(storage: 0b11111000_00000000_00000001_00000000)
    ...
    init(run: Int, distance: Int) {
        ...
        let symbols: UInt32 = .init(decade.distance) << 27 | 0x0000_0100 | .init(decade.run)
        let bits: UInt32 = (.init(distance) - base.distance) << 14 | (.init(run) - base.run) <<  9
