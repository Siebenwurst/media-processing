extension LZ77 {
    ...
    struct DeflatorMatches {
        private var storage: ManagedBuffer<Void, UInt32>

        let capacity: Int
        private(set) var count: Int
        private var limit: Int

        private var depths: Depths
    }
}
...
extension LZ77.DeflatorMatches {
    ...
    static func graph(capacity: Int) -> Self {
        .init(capacity: capacity << 5, limit: min(2048, capacity))
    }

    static func terms(capacity: Int) -> Self {
        .init(capacity: capacity, limit: min(1024, capacity))
    }

    private init(capacity: Int, limit: Int) {
    ...
    var unfilled: Int { self.limit - 1 - self.count }
    ...
    mutating func store(literal: UInt8) {
        assert(self.unfilled > 0)

        self[offset: self.endIndex] = LZ77.DeflatorTerm.init(literal: literal).storage
        self.count += 1
    }

    mutating func store(match: (run: Int, distance: Int)) {
        assert(self.unfilled > 0)

        self[offset: self.endIndex] = LZ77.DeflatorTerm.init(run: match.run, distance: match.distance).storage
        self.count += 1
    }


    mutating func resetTerms() {
        self.count = 0
    }

    mutating func trees() -> (runliteral: LZ77.HuffmanTree<UInt16>, distance: LZ77.HuffmanTree<UInt8>) {
    ...
    @discardableResult
    mutating func store(vertex literal:UInt8) -> Int {
        assert(self.unfilled > 0)
    ...
    mutating func set(edge: (run: Int, distance: Int), at base: Int) {
        // must be one empty space at the end to be the sink node
        assert(base >> 5 < self.limit - 1)

        let position = base + .init(LZ77.Decades[distance: edge.distance])
        let candidate = UInt32(edge.run)
    ...
    mutating func trees(iterations: Int) -> (runliteral: LZ77.HuffmanTree<UInt16>, distance: LZ77.HuffmanTree<UInt8>) {
    ...
    private mutating func minimize() -> [Int] {
        ...
        repeat {
            ...
            self[offset: next.index << 5] = current.upstream & 0xff_ff_ff_00 | next.upstream & 0x00_00_00_ff
    ...
    private mutating func explore(from index: Int) {
        ...
        for decade: UInt8 in 0 ..< 30 {
            ...
            for length: Int in 3 ... maxlength {
                ...
                guard depth < next.depth else {
                    continue
                }

                let slug: UInt32 = .init(length) << 16 | .init(decade) << 8
                self[index + length] = (upstream: slug | next.upstream & 0x00_00_00_ff, depth: depth)
            }
        }
    }
}
