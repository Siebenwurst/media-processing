extension LZ77.InflatorBuffers {
    ...
    struct Stream {
        ...
        var statistics: (literals: [Int], matches: [Int], symbols: [Int]) = (
...
extension LZ77.InflatorBuffers.Stream {
    mutating func push(_ data: ArraySlice<UInt8>) {
        self.input.rebase(data, pointer: &self.b)
    }

    mutating func pull(_ count: Int) -> [UInt8]? {
        self.output.exclude()
...
extension LZ77.InflatorBuffers.Stream {
    mutating func readBlockMetadata(into metadata: inout LZ77.BlockMetadata) throws -> LZ77.BlockShape? {
        guard self.b + 3 <= self.input.count else {
            return nil
        }

        // read block header bits
        let final = self.input[self.b, count: 1, as: UInt8.self] != 0
        switch self.input[self.b + 1, count: 2, as: UInt8.self] {
        case 0:
            // skip to next byte boundary, read 4 bytes
            let boundary = (self.b + 3 + 7) & ~7
            guard boundary + 32 <= self.input.count else {
                return nil
            }
        ...
        case 2:
            guard self.b + 17 <= self.input.count else {
                return nil
            }

            let codelengths = 4 + self.input[self.b + 13, count: 4, as: Int.self]

            guard self.b + 17 + 3 * codelengths <= self.input.count else {
                return nil
            }

            let literals = 257 + self.input[self.b +  3, count: 5, as: Int.self]
            let distances = 1 + self.input[self.b +  8, count: 5, as: Int.self]
            // other counts don’t need to be checked because the number of bits
            // matches the acceptable range
            guard 257 ... 286 ~= literals else {
                throw LZ77.DecompressionError.invalidHuffmanRunLiteralSymbolCount(literals)
            }

            var lengths: [Int] = .init(repeating: 0, count: 19)
            for (i, d): (Int, Int) in zip(0 ..< codelengths, [16, 17, 18, 0, 8, 7, 9, 6, 10, 5, 11, 4, 12, 3, 13, 2, 14, 1, 15]) {
                lengths[d] = self.input[self.b + 17 + 3 * i, count: 3, as: Int.self]
            }
            guard let tree: LZ77.HuffmanTree<UInt8> = .validate(symbols: 0 ... 18, lengths: lengths) else {
    ...
    mutating func readBlockTables(
        metadata: LZ77.BlockMetadata,
        lengths count: (literals:Int, total:Int),
        reusing lengths: inout [Int]
    ) throws -> LZ77.InflatorTables? {
        // code lengths form an unbroken sequence
        codelengths: while lengths.count < count.total {
            guard self.b < self.input.count else {
                return nil
            }
            ...
            guard self.b + meta.length <= self.input.count else {
                return nil
            }
            ...
            //          (7 bits of length)
            let element: Int, extra: Int, base: Int
            switch meta.symbol {
            case 0 ..< 16:
            ...
            guard self.b + meta.length + extra <= self.input.count else {
                return nil
            }
            let repetitions = base + self.input[self.b + meta.length, count: extra, as: Int.self]
        ...
        for (i, length): (Int, Int) in self.lengths.prefix(runliterals).enumerated() where length > 0 {
            print("    [\(String.pad("\(i)", left: 3))]: \(length)")
        }
        print("< dynamic distance codelengths:")
        for (i, length):(Int, Int) in self.lengths.dropFirst(runliterals).enumerated() where length > 0 {
            print("    [\(String.pad("\(i)", left: 3))]: \(length)")
        }
        #endif

        guard let literalTree: LZ77.HuffmanTree<UInt16> = .validate(symbols: 0 ... 287, lengths: lengths.prefix(count.literals)),
            let distanceTree: LZ77.HuffmanTree<UInt8> = .validate(symbols: 0 ... 31, normalizing: lengths.dropFirst(count.literals)) else {
    ...
    mutating func readBlock(with tables: LZ77.InflatorTables) throws -> Void? {
        while self.b < self.input.count {
            ...
            if runliteral.symbol < 256 {
                guard self.b + runliteral.length <= self.input.count else {
                    return nil
                }
                ...
                // of checked shifts
                var slug: UInt64 = .init(self.input[self.b + 32]) << 32 | .init(self.input[self.b + 16]) << 16 | .init(first)
                slug &>>= runliteral.length

                let composite: (count: (extra: Int, base: Int), offset: (extra: Int, base: Int))

                composite.count = tables.composite(decade: runliteral)
                let count: Int = composite.count.base &+ .init(truncatingIfNeeded: slug & ~(.max &<< composite.count.extra))

                slug &>>= composite.count.extra

                let distance: LZ77.Distance = tables[.init(truncatingIfNeeded: slug), as: LZ77.Distance.self]
                slug &>>= distance.length

                composite.offset = tables.composite(decade: distance)
                let offset: Int = composite.offset.base &+ .init(truncatingIfNeeded: slug & ~(.max &<< composite.offset.extra))

                let b: Int = self.b + runliteral.length + composite.count.extra + distance.length + composite.offset.extra
                guard b <= self.input.count else {
                    return nil
                }

                guard self.output.endIndex - offset >= self.output.startIndex else {
                ...
                let m: Int = runliteral.length + composite.count.extra + distance.length + composite.offset.extra
    ...
    mutating func readBlock(upTo end: Int) -> Void? {
    ...
    mutating func readBigEndianUInt32() -> UInt32? {
        // skip to next byte boundary, read 4 bytes
        ...
        let checksum: UInt32 = bytes.0 << 24 | bytes.1 << 16 | bytes.2 <<  8 | bytes.3

        return checksum
    }

    mutating func readLittleEndianUInt32() -> UInt32? {
        self.readBigEndianUInt32()?.byteSwapped
    }

    mutating func readString() -> Void? {
        reading: do {
    ...
    mutating func readByte() -> UInt8? {
        guard self.b + 8 <= self.input.count else {
            return nil
        }
        defer {
            self.b += 8
    ...
    mutating func _dumpPerfStats() {
        #if DUMP_LZ77_BLOCKS
        let efficiency: Double = self.statistics.literals.enumerated().reduce(0.0){ $0 + .init($1.0 * $1.1) } / .init(self.statistics.literals.reduce(0, +))
        ...
        #endif
        #if DUMP_LZ77_BLOCKS || DUMP_LZ77_SYMBOL_HISTOGRAM
        print(String(histogram: self.statistics.symbols, size: (29, 30), pad: 4))
        #endif
    }
}
