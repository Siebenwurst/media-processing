extension LZ77 {
    ...
    struct HuffmanTree<Symbol> where Symbol: Comparable {
        ...
        private init(symbols: [Symbol], levels: [Range<Int>], size: (n: Int, z: Int)) {
...
extension LZ77.HuffmanTree<UInt16> {
    static var runliteral: Self {
        .init(
            symbols: [256 ... 279, 0 ... 143, 280 ... 287, 144 ... 255].flatMap { $0 },
            levels: .init(repeating: 0 ..< 0, count: 6) + // L1 ... L6
...
extension LZ77.HuffmanTree<UInt8> {
    static var distance: Self {
        .init(
            symbols: .init(0 ... 31),
            levels: .init(repeating:  0 ..<  0, count:  4) + [0 ..< 32] + .init(repeating: 32 ..< 32, count: 10)
        )
    }
}
...
extension LZ77.HuffmanTree where Symbol: BinaryInteger {
    ...
    init(symbols: [Symbol], levels: [Range<Int>]) {
        guard let size: (n: Int, z: Int) = Self.size(levels) else {
    ...
    private static func size(_ levels:[Range<Int>]) -> (n:Int, z:Int)? {
    ...
    static func validate<Symbols, Lengths>(
        symbols: Symbols,
        normalizing lengths: Lengths
    ) -> Self? where Symbols: Collection, Lengths: Collection, Symbols.Element == Symbol, Lengths.Element == Int {
        var first: Symbol?
    ...
    static func validate<Symbols, Lengths>(
        ...
        guard let size: (n: Int, z: Int) = Self.size(ranges) else {
            return nil
        }
...
extension LZ77.HuffmanTree where Symbol: BinaryInteger {
    func codewords(
        initializing destination: UnsafeMutablePointer<LZ77.Codeword>,
        count: Int,
        extra: (Symbol) -> UInt8
    ) {
    ...
    func mass<C>(frequencies: C) -> Int where C: RandomAccessCollection, C.Index == Int, C.Element == Int {
        var total: Int = 0
    ...
    init<C>(frequencies: C, limit: Int) where C: RandomAccessCollection, C.Index == Int, C.Element == Int {
    ...
    private static func limitHeight<S>(_ uncompacted:S, to height:Int) -> [Int] where S:Sequence, S.Element == Int {
        var levels: [Int] = .init(uncompacted)
        guard levels.count > height else {
            return levels
        }
